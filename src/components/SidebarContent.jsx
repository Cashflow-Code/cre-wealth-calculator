...
export default function SidebarContent({
  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6">
      ...
      <button
        onClick={onReset}
        className="w-full flex items-center justify-center gap-2 py-2 rounded-xl border border-slate-700/50 bg-slate-800/50 text-slate-400 hover:text-emerald-400 hover:border-emerald-500/30 hover:bg-slate-800 text-xs font-semibold transition-colors"
      >
      ...
      <section className="space-y-4">
        ...
        {/* Effective rate read-out (replaces flat-rate slider) */}
        <div className="rounded-lg border border-slate-700/40 bg-slate-800/30 px-3 py-2.5 space-y-1.5">
          <div className="flex items-center justify-between gap-2">
            <span className="text-[10px] text-slate-500 font-medium">Federal effective rate</span>
            <span className="text-xs font-bold text-slate-300 tabular-nums">{(federalEffective * 100).toFixed(1)}%</span>
          </div>
          <div className="flex items-center justify-between gap-2">
            <span className="text-[10px] text-slate-500 font-medium">Combined (fed + state)</span>
            <span className="text-xs font-bold text-emerald-400 tabular-nums">{(totalEffective * 100).toFixed(1)}%</span>
          </div>
        </div>
      ...
      {/* Loan */}
      <section className="border-t border-slate-700/40 pt-5 space-y-4">
        <SectionHeader icon={Building2} label="Loan / Leverage" iconColor="text-violet-400" />
        <Slider label="LTV (Loan-to-Value)" value={ltv} onChange={setLtv}
          min={0} max={80} step={5} format={(v) => (v === 0 ? 'None (all cash)' : `${v}%`)}
          sublabel={ltv === 0 ? 'No debt — matches original model' : `${100 - ltv}% down per property`} />
        {ltv > 0 && (
          <>
