...
export default function WealthChart({
  ...
  yearsToReach, totalYears, showStockAlt,
}) {
  return (
    <div className="rounded-2xl border border-slate-700/40 bg-[#0c1428]/80 p-5 shadow-xl">
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h3 className="text-sm font-bold uppercase tracking-widest text-slate-300">Wealth Trajectory</h3>
      ...
      <ResponsiveContainer width="100%" height={480}>
        <ComposedChart data={data} margin={{ top: 30, right: 20, left: 0, bottom: 0 }}>
          ...
          <XAxis dataKey="year" stroke="#475569" fontSize={11} interval={0} />
          <YAxis tickFormatter={(v) => fmt(v)} stroke="#475569" fontSize={11} width={72} />
          ...
          <ReferenceLine
            ...
            label={{ value: 'Tax savings start', fill: '#a78bfa', fontSize: 10, position: 'top' }}
          />
          <ReferenceLine
            x={`Y${buyingYears}`}
            stroke="#f59e0b" strokeDasharray="4 4"
            label={{ value: 'Buying ends', fill: '#f59e0b', fontSize: 10, position: 'top' }}
          />
          {isReachable && yearsToReach <= totalYears && (
            <ReferenceLine
              x={`Y${yearsToReach}`}
              stroke="#10b981" strokeDasharray="2 2"
              label={{ value: 'Financial Freedom', fill: '#10b981', fontSize: 10, position: 'top' }}
            />
          )}
