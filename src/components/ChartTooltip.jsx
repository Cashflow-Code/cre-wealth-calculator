...
export default function ChartTooltip({ active, payload, showStockAlt }) {
  ...
  return (
    <div className="bg-[#0d1630]/95 border border-slate-700/60 rounded-xl p-3 shadow-2xl backdrop-blur-sm min-w-[260px]">
      <p className="text-xs font-bold text-slate-300 mb-2">
        Year {d.yearNum}{' '}
        {d.isBuyingPhase
          ? <span className="text-emerald-400">· Buying</span>
          : <span className="text-sky-400">· Holding</span>}
      </p>
      <div className="space-y-1 text-xs mb-2">
        <div className="flex items-center justify-between gap-4 pb-1 border-b border-slate-700/50">
          ...
          <span className="font-bold text-emerald-400 tabular-nums">{fmt(d.totalProfits)}</span>
        </div>
        <div className="flex justify-between gap-4 text-slate-400 pl-3">
          <span>Equity (appreciation)</span>
          <span className="tabular-nums">{fmt(d.equity)}</span>
        </div>
        <div className="flex justify-between gap-4 text-slate-400 pl-3">
      ...
      {showStockAlt && (
        <div className="flex items-center justify-between gap-4 text-xs pt-2 border-t border-slate-700/50">
          <span className="text-sky-400 flex items-center gap-1 font-semibold">
            <BarChart3 className="w-3 h-3" /> Just Stocks
          </span>
          <span className="font-bold text-sky-400 tabular-nums">{fmt(d.stockBalance)}</span>
        </div>
      )}
      <div className="flex items-center justify-between gap-4 text-xs pt-2 border-t border-slate-700/50">
        ...
        <span className="font-bold text-red-400 tabular-nums">{fmt(d.cumulativeTaxesPaid)}</span>
      </div>
    </div>
  );
}
