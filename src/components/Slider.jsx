  ...
  return (
    <div className={`space-y-1.5 transition-opacity ${disabled ? 'opacity-40 pointer-events-none' : ''}`}>
      <div className="flex items-baseline justify-between gap-2">
        ...
        <span className="text-sm font-bold text-emerald-400 tabular-nums">{format(value)}</span>
      </div>
      <input
        type="range"
        value={value}
        ...
        step={step}
        disabled={disabled}
        className="w-full h-1.5 bg-slate-800 rounded-full appearance-none cursor-pointer accent-emerald-500 disabled:cursor-not-allowed"
      />
